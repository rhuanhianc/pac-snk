...
const createSnake = (chain, { sizeCell, sizeDot, colorSnake }, duration // Duration for movement animation
) => {
    ...
    const styles = [
      ...
      50% { clip-path: polygon(50% 50%, 100% 0%, 0% 0%, 0% 100%, 100% 100%, 50% 50%); }
        ...
        ...snakeParts.slice(0, partsToRender).map((positions, i) => {
            ...
            const isHead = i === 0;
            const keyframes = removeInterpolatedPositions(positions.map((tr, i, { length }) => ({ ...tr, t: i / length }))).map(({ t, ...p }) => {
                if (isPacman && isHead && t < 1) {
                    const nextIndex = Math.min(Math.floor(t * positions.length) + 1, positions.length - 1);
                    if (nextIndex > Math.floor(t * positions.length) &&
